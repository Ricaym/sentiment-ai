    ...
    stages {
        ...
        stage('SonarQube Analysis') {
            ...
            steps {
                ...
                sh '''
                    docker run --rm --network cicd-network --volumes-from jenkins -w "$WORKSPACE" -e SONAR_HOST_URL="$SONAR_HOST_URL" -e SONAR_TOKEN="$SONARQUBE_TOKEN" sonarsource/sonar-scanner-cli:latest sonar-scanner -Dsonar.projectKey=sentiment-ai -Dsonar.projectName=SentimentAI -Dsonar.projectBaseDir="$WORKSPACE" -Dsonar.sources=src -Dsonar.python.version=3.11 -Dsonar.python.coverage.reportPaths=coverage.xml -Dsonar.sourceEncoding=UTF-8 -Dsonar.scanner.metadataFilePath=$WORKSPACE/report-task.txt
                '''
                }
            }
        }
        stage('Quality Gate') {
            steps {
                timeout(time: 15, unit: 'MINUTES') {
                    // Attend le r é sultat asynchrone du Quality Gate SonarQube
                    // abortPipeline : true = > bloque Push et Deploy si le gate é choue
