...
### 1. Installer Jenkins via Docker
Jenkins sera installé dans un conteneur Docker qui a accès au daemon Docker de l’hôte. C’est la
technique Docker-out-of-Docker (DooD) : Jenkins ne fait pas tourner son propre daemon Docker,
il pilote celui de la machine hôte via le socket Unix /var/run/docker.sock.
1.1 Lancer Jenkins
